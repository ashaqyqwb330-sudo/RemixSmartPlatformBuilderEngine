...
object BuildPackExporter {
    ...
    fun wrapTextWithMode(context: Context, text: String): Pair<String, Int> {
        ...
        return when (mode) {
            "smart" -> {
                val blocks = com.example.engine.SmartCaptureEngine.decomposeText(text)
                if (blocks.isEmpty()) {
                    return Pair("", 0)
                }
                ...
                for ((index, block) in blocks.withIndex()) {
                    ...
                    val cleanTitle = sanitizedTitle.replace(Regex("\\s+"), "_").trim().ifEmpty { "block_$index" }
                    
                    val ext: String
                    val content: String
                    
                    if (block.type == com.example.engine.ContentType.TEXT && convertMdToHtml) {
                        ext = "html"
                        content = com.example.engine.SmartCaptureEngine.generateHtmlWrapper(title, "نص", block.content, theme)
                    } else if (block.type == com.example.engine.ContentType.HTML) {
                        ext = "html"
                        content = block.content
                    } else if (block.type == com.example.engine.ContentType.TEXT) {
                        ext = "md"
                        content = block.content
                    } else {
            ...
            "raw" -> {
                ...
                var rawTitle = if (match != null) {
                    match.groupValues[1].trim()
                } else {
                    text.take(50).replace("\n", " ").trim()
                }
                if (rawTitle.isEmpty()) {
                    rawTitle = "bundled_document"
                }
                ...
                if (text.contains("<html>", ignoreCase = true) || text.contains("<title>", ignoreCase = true)) {
                    ext = "html"
                    content = text
                } else {
                    if (convertMdToHtml) {
                        ext = "html"
                        content = com.example.engine.SmartCaptureEngine.generateHtmlWrapper(rawTitle, "نص", text, theme)
                    } else {
                        ext = "md"
                        content = text
                    }
